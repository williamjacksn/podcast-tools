...
def make_podcast_xml(podcast_info: Dict) -> ETree.Element:
    ...
    link_self = ETree.SubElement(
        channel, "atom:link", href=f"{base_url}_.xml", rel="self"
    )
    link_self.tail = "\n    "
...
def add_item_to_podcast(
    rss: ETree.Element, podcast_info: Dict, mp3_info: Dict, index: int
):
...
def main():
    ...
    if saved_info_path.exists():
        with saved_info_path.open() as f:
            info = json.load(f)
    info["base_url"] = (
        input("Base url [{base_url}]: ".format(**info)) or info["base_url"]
    )
    info["title"] = input("Podcast title [{title}]: ".format(**info)) or info["title"]
    info["author"] = (
        input("Podcast author [{author}]: ".format(**info)) or info["author"]
    )
    info["description"] = (
        input("Podcast description [{description}]: ".format(**info))
        or info["description"]
    )
    info["owner_email"] = (
        input("Podcast owner email [{owner_email}]: ".format(**info))
        or info["owner_email"]
    )
